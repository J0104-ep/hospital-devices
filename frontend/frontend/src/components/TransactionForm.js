...
import "./TransactionForm.css"; // 👈 crea este archivo si no lo tienes
...
export default function TransactionForm() {
  ...
  const loadEquipments = async () => {
    ...
    if (!token) {
      setError("No token provided. Inicia sesión nuevamente.");
      return;
    }
  ...
  // 🖼️ Seleccionar equipo desde el grid
        ...
        <div className="form-group">
          ...
          <div className="equipment-grid">
            {equipments.map((eq) => (
              ...
              >
                <img
                  src={`http://localhost:4000${eq.imageUrl}`}
                  alt={eq.type}
